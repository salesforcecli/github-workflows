...
import { log, setOutput, getExtensionInfo, compareSemver, extractVersionFromTag, } from '../core/utils.js';
/**
 * Get all available VS Code extensions (packages with publisher field)
 */
function getAvailableExtensions() {
    const extensions = [];
    const packagesRoot = process.env.PACKAGES_ROOT || 'packages';
    const packagesDir = join(process.cwd(), packagesRoot);
        ...
        if (existsSync(packageJsonPath)) {
            try {
                ...
                if (info.publisher) {
                    ...
                    log.debug(`Found VS Code extension: ${packageName} (publisher: ${info.publisher})`);
                }
                else {
 ...
 */
async function hasExtensionChanges(git, extensionPath, lastTag) {
    if (!lastTag) {
 ...
 */
async function findLastReleaseTagForExtension(git, extensionName) {
    try {
        const tags = await git.tags();
        const allTags = tags.all
            .filter((tag) => tag.startsWith(`${extensionName}-v`))
            .map((tag) => {
            const version = extractVersionFromTag(tag);
            return { tag, version };
        });
        const extensionTags = allTags
 ...
 */
function intersectExtensions(userSelected, changedExtensions, availableExtensions, buildContext) {
    ...
    const validUserSelected = userSelected.filter((ext) => {
        if (!availableNames.includes(ext)) {
            log.warning(`User selected extension '${ext}' is not available - skipping`);
            return false;
        }
        return true;
    });
    ...
    if (buildContext.isNightly || buildContext.isPromotion) {
        ...
        log.info(`${buildType} build - using user selection: ${validUserSelected.join(', ')}`);
        return validUserSelected;
    }
    // For regular builds, intersect user selection with detected changes
    const intersection = validUserSelected.filter((ext) => changedExtensions.includes(ext));
 ...
 */
async function detectBumpTypeFromCommits(git, extensionPath, lastTag) {
    try {
        ...
        const messages = log_.all.map((c) => c.message);
        let bump = 'patch';
 ...
 */
export async function detectExtensionChanges(buildContext, promotionCommitSha, userSelectedExtensions) {
    ...
    log.info(`Found ${extensions.length} extensions: ${extensions.map((e) => e.name).join(', ')}`);
    ...
    // For nightly and regular builds, check for changes since last release
        ...
        for (const extension of extensions) {
            ...
            if (lastTag) {
                log.info(`Comparing ${extension.name} against last release tag: ${lastTag}`);
            }
            else {
                log.info(`No previous release tag found for ${extension.name} - treating as first release`);
            }
            const hasChanges = await hasExtensionChanges(git, extension.path, lastTag);
            if (hasChanges) {
                log.info(`Found changes in ${extension.name} - including in ${buildType.toLowerCase()} release`);
                ...
                if (buildContext.isNightly &&
                    ...
                    const detectedBump = await detectBumpTypeFromCommits(git, extension.path, lastTag);
                    if (detectedBump === 'major' ||
                        (detectedBump === 'minor' && versionBumps !== 'major')) {
                        log.info(`Upgrading bump type for ${extension.name}: ${versionBumps} → ${detectedBump} (conventional commits)`);
                        versionBumps = detectedBump;
                    }
                }
            }
            else {
                log.info(`No changes found in ${extension.name} - skipping ${buildType.toLowerCase()} release`);
            }
        }
    }
    // Intersect user selection with detected changes
    const finalSelectedExtensions = intersectExtensions(userSelected, changedExtensions, extensions, buildContext);
 ...
 */
export function setChangeDetectionOutputs(result) {
...
export async function main() {
    try {
        ...
        const versionBump = process.env.VERSION_BUMP || 'auto';
        ...
        log.info(`Raw SELECTED_EXTENSIONS env var: "${process.env.SELECTED_EXTENSIONS}"`);
        const buildContext = {
            isNightly,
            versionBump,
            preRelease,
        ...
        const result = await detectExtensionChanges(buildContext, promotionCommitSha, userSelectedExtensions);
...
//# sourceMappingURL=ext-change-detector.js.map