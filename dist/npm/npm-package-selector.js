...
import { log, setOutput, getExtensionInfo } from '../core/utils.js';
/**
 * Get all available NPM packages
 */
export function getAvailableNpmPackages() {
    log.info('Getting all available NPM packages...');
    // Get all packages from the packages directory (configurable via PACKAGES_ROOT)
        ...
        if (existsSync(packageJsonPath)) {
            try {
                ...
                if (!info.publisher) {
                    ...
                    log.debug(`Skipping VS Code extension: ${packageName} (publisher: ${info.publisher})`);
                }
            }
            catch (error) {
 ...
 */
function parseUserSelectedPackages(selectedPackagesInput) {
 ...
 */
function intersectPackages(userSelected, changedPackages, availablePackages) {
    ...
    const intersection = validUserSelected.filter((pkg) => changedPackages.includes(pkg));
 ...
 */
export function selectNpmPackages(userSelectedPackages, availablePackages, changedPackages) {
    ...
    // Intersect user selection with detected changes
    const finalSelectedPackages = intersectPackages(userSelected, changed, available);
 ...
 */
export function setPackageSelectionOutputs(selectedPackages) {
 ...
 */
export function setPackageDiscoveryOutputs(npmPackages) {
...
export async function main() {
    try {
        ...
        if (userSelectedPackages || availablePackages || changedPackages) {
            ...
            const selectedPackages = selectNpmPackages(userSelectedPackages, availablePackages, changedPackages);
...
//# sourceMappingURL=npm-package-selector.js.map