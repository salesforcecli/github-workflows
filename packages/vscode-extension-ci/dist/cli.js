...
import { findNightlyCandidate, setNightlyFinderOutputs, } from './extension/ext-nightly-finder.js';
import { detectExtensionChanges, setChangeDetectionOutputs, } from './extension/ext-change-detector.js';
import { getAvailableExtensions, setExtensionDiscoveryOutputs, } from './extension/ext-package-selector.js';
import { detectNpmChanges, setNpmChangeDetectionOutputs, } from './npm/npm-change-detector.js';
import { npmPackageSelectorMain } from './npm/npm-package-selector.js';
import { extractPackageDetails, setPackageDetailsOutputs, } from './npm/npm-package-details.js';
...
program
    ...
    .description('Discover available NPM packages or select packages based on user input')
    .action(async () => {
    try {
...
program
    ...
    try {
        createGitHubReleases({
            ...
            selectedExtensions: process.env.SELECTED_EXTENSIONS || '',
            isNightly: process.env.IS_NIGHTLY || 'false',
            vsixArtifactsPath: process.env.VSIX_ARTIFACTS_PATH || './vsix-artifacts',
        });
    }
    catch (error) {
