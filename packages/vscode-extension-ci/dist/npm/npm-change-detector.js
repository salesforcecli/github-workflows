 ...
 */
function getAvailableNpmPackages() {
    const packages = [];
 ...
 */
async function hasPackageChanges(git, packagePath, baseBranch) {
    try {
 ...
 */
async function determineVersionBump(git) {
    try {
        ...
        if (commitMessages.toLowerCase().includes('breaking') ||
        ...
        log.warning(`Failed to determine version bump: ${error}, defaulting to patch`);
        return 'patch';
    }
}
/**
 ...
 */
export async function detectNpmChanges(baseBranch = 'main') {
    ...
    try {
        ...
        const baseBranchExists = branches.all.some((branch) => branch.includes(`origin/${baseBranch}`));
        if (!baseBranchExists) {
            log.warning(`Base branch 'origin/${baseBranch}' does not exist, falling back to 'main'`);
            baseBranch = 'main';
        }
    }
    catch (error) {
    ...
    log.info(`Found ${npmPackages.length} NPM packages: ${npmPackages.map((p) => p.name).join(', ')}`);
 ...
 */
export function setNpmChangeDetectionOutputs(result) {
...
//# sourceMappingURL=npm-change-detector.js.map