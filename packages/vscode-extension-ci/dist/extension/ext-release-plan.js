...
function displayReleasePlan(options) {
    const { branch = 'main', buildType, isNightly, versionBump, registries, preRelease, selectedExtensions, } = options;
    ...
    for (const ext of extensions) {
        ...
        if (isNightly === 'true') {
            console.log('  Version strategy: Nightly build (odd minor + nightly timestamp)');
        }
        else {
            console.log(`  Version strategy: ${versionBump} (conventional commit) + VS Code even/odd (pre-release: ${preRelease})`);
        }
        ...
        const registryList = registries === 'all'
            ? ['vsce', 'ovsx']
        ...
        for (const registry of registryList) {
            switch (registry) {
                case 'vsce':
                    console.log(`  Would publish to: VSCode Marketplace${preReleaseText}`);
                    break;
                case 'ovsx':
...
//# sourceMappingURL=ext-release-plan.js.map