...
function calculateNewVersion(currentVersion, versionBump, isNightly, isPromotion, preRelease) {
    ...
    if (isNightly) {
        ...
        if (minor % 2 === 1) {
            ...
            console.warn('Warning: Current version has even minor, expected odd for promotion');
...
function createGitTag(packageName, version, isPreRelease, promotionCommitSha, isNightly) {
    ...
    try {
        ...
        if (promotionCommitSha) {
            ...
            console.log(`Creating tag ${tagName} on promotion commit ${promotionCommitSha}...`);
...
function bumpVersions(options) {
    const { versionBump, selectedExtensions, preRelease, isNightly, isPromotion, promotionCommitSha, } = options;
    ...
    for (const ext of extensions) {
        ...
        const newVersion = calculateNewVersion(packageDetails.version, versionBump, isNightly === 'true', isPromotion === 'true', preRelease === 'true');
        console.log(`🔄 Bumping ${ext} from ${packageDetails.version} to ${newVersion}`);
        ...
        try {
            ...
            createGitTag(packageDetails.name, newVersion, preRelease === 'true', promotionCommitSha, isNightlyBuild);
        }
        catch (error) {
...
export { bumpVersions };
//# sourceMappingURL=ext-version-bumper.js.map