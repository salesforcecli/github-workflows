...
function getPackageDetails(extensionPath) {
    try {
        const packageJsonPath = join(process.cwd(), 'packages', extensionPath, 'package.json');
        const content = readFileSync(packageJsonPath, 'utf-8');
        return JSON.parse(content);
    }
    catch (error) {
        console.warn(`Warning: Could not read package.json for ${extensionPath}:`, error);
        return null;
    }
}
function findVsixFiles(extension, artifactsPath) {
    try {
...
function generateReleaseNotes(extension, currentVersion, isNightly, preRelease) {
    let releaseNotes = `## ${extension} v${currentVersion}\n\n`;
    releaseNotes += '### Changes\n\n';
    try {
        // Find the last release tag for this extension
        const lastTag = execSync('git tag --sort=-version:refname | grep "^v" | head -1', { encoding: 'utf8' }).trim();
        if (lastTag) {
            // Get commits since the last release
            const recentCommits = execSync(`git log --oneline "${lastTag}"..HEAD -- "packages/${extension}/"`, { encoding: 'utf8' }).trim();
            if (recentCommits) {
            ...
            const allCommits = execSync(`git log --oneline -- "packages/${extension}/"`, { encoding: 'utf8' }).trim();
            if (allCommits) {
        ...
        console.warn(`Warning: Could not generate release notes for ${extension}:`, error);
        releaseNotes += '- General improvements and bug fixes\n';
    }
    releaseNotes += '\n### Installation\n\n';
...
function createGitHubRelease(extension, currentVersion, releaseNotes, vsixFiles, isNightly, preRelease, dryRun) {
    // Create release tag
    ...
    if (isNightly === 'true') {
        ...
        const branchSuffix = branch === 'main' ? '' : `.${branch.replace(/\//g, '-')}`;
    ...
    if (dryRun) {
        ...
        try {
            ...
            let hasAssets = false;
            try {
                const viewOutput = execSync(`gh release view "${releaseTag}" --repo "${repo}" --json assets`, { encoding: 'utf8' });
            ...
            if (releaseExists && hasAssets) {
                console.log(`⏭️ Release ${releaseTag} already exists with assets — skipping`);
            }
            else if (releaseExists) {
                console.log(`📎 Release ${releaseTag} exists but has no assets — uploading`);
                execSync(`gh release upload "${releaseTag}" ${vsixArgs} --repo "${repo}"`, { stdio: 'inherit' });
                ...
                const command = `gh release create "${releaseTag}" --title "${releaseTitle}" ` +
...
function createGitHubReleases(options) {
    const { dryRun, preRelease, versionBump, selectedExtensions, isNightly, vsixArtifactsPath, } = options;
    ...
    for (const ext of extensions) {
        ...
        const releaseNotes = generateReleaseNotes(ext, packageDetails.version, isNightly, preRelease);
        createGitHubRelease(ext, packageDetails.version, releaseNotes, vsixFiles, isNightly, preRelease, dryRun);
    }
    if (dryRun) {
...
//# sourceMappingURL=ext-github-releases.js.map